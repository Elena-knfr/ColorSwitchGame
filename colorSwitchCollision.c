...
bool collided () {
	float distance;
		/*for (int i=0; i<sizeof(obstacle_array1)/sizeof(obstacle_array1[0]); i++){
			//ABS(y_obstacle[2] - y_player) < r_player + r_obstacle[2]
			//if ((x2-x1)*(x2-x1))+((y2-y1)*(y2-y1))
			if (two_dis(obstacle_array1[i][0], obstacle_array1[i][1], 150, y_player) < r_player){
				//if (obstacle_array1[i][2] != color_player)
					return true;
			}
